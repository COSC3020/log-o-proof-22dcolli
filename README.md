...
Through these processes we see that that when starting with $T(n) \leq c * \log_{2}(n)$ can transform into $T(n) \leq c * \log_{5}(n)$ and starting with $T(n) \leq c * \log_{5}(n)$ can transform into $T(n) \leq c * \log_{2}(n)$. Meaning that we have proven that $O(\log_{2} n)$ is the same as $O(\log_{5} n)$
